...
class CalendarDataManager{ // 6*7배열에 나타낼 달력 값을 구하는 class
	...
	public void setToday(){
		...
		calMonth = today.get(Calendar.MONTH); //1월이면 0변환
	...
	private void makeCalData(Calendar cal){
		...
		//((5+7-6)%7)%7 //0이면 1일이 일요일, 5이면 금요일에 1이 시작
		System.out.println(calMonth);
		
		if(calMonth == 1) calLastDate = calLastDateOfMonth[calMonth] + leapCheck(calYear); //윤년이면 2월은 29일까지
